...
# Se estiver usando xhtml2pdf (comum no Render para gerar PDFs)
# Instale via: pip install xhtml2pdf
...
def listar_membros(request):
    # Correção: usando 'nome_completo' conforme seu modelo
...
def registrar_frequencia_reuniao(request, pk):
    reuniao = get_object_or_404(Reuniao, pk=pk)
    # CORREÇÃO FieldError: Usando 'nome_completo' em vez de 'nome'
...
def historico_frequencia(request):
    ...
    # Organiza os dados para o template do histórico
...
def historico_frequencia_pdf(request):
    ...
    # Busca as células que possuem frequência registrada dentro dos filtros
    ...
    for celula in celulas_list:
        # Busca membros desta célula que possuem frequências nos critérios
